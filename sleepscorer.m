...
if(length(EMG_TIMESTAMPS)-(EPOCH_StartPoint+EPOCHSIZE) <= 0)
    ...
    plot_epochdata_on_axes_in_GUI_08112017;
...
plot_epochdata_on_axes_in_GUI_08112017(num2str(start_point));
SLIDERVALUE=(100*EPOCH_StartPoint)/length(EMG_TIMESTAMPS);
set(handles.slider1, 'Value',SLIDERVALUE); 
moveLeft=moveLeft + 1;
guidata(handles.sleepscorer,handles);
...
if isequal(FileFlag, 2) % AD System
    ...
    switch ext
        ...
        case '.emg'
            ...
            filtered_samples=(filtfilt(Bhigh,Ahigh,EMG_SAMPLES))/4;
            ...
            if EMG_lowpass_enable>0
                ...
                filtered_samples = filtfilt(EMG_Blow,EMG_Alow, filtered_samples);
            end
...
elseif isequal(FileFlag, 1) % Neuralynx System
    ...
    waitbar(0.4,waithandle,'Filtering the EMG data ...'); 
    figure(waithandle),pause(0.2),
    if EMG_lowpass_enable>0 % OPTIONAL lowpass filter for EMG signals
        if EMG_LP_Fc > Fs/2
            msgbox({['Down-Sampled Sampling Rate:  ' num2str(Fs) 'Hz'];...
            ['You have exceeded the max low pass setting.']; ['New low pass frequency is:  ' num2str(DSampSF(indSampfactor(end))/2) '']});
            EMG_LP_Fc = Fs/2; %Max allowed low pass frequency based on Nyquist rule         
        end
    else
        EMG_LP_Fc = floor(Fs/2)-1; %Max allowed low pass frequency based on Nyquist rule
    end
    %Design bandpass filter:
    % Default setting implements high pass = 30hz
    [z, p, k] = ellip(7,1,60, [EMG_Fc EMG_LP_Fc]/(SampFreq1/2),'bandpass');
    [sos, g] = zp2sos(z,p,k);
    filtered_samples = filtfilt(sos,g, EMG_SAMPLES);
...
%     end
    ...
    waitbar(0.4,waithandle,'Filtering the EMG data ...'); 
    figure(waithandle),pause(0.2),
    filtered_samples = filtfilt(Bhigh,Ahigh,EMG_SAMPLES);
    % OPTIONAL lowpass filter for EMG signals
    if EMG_lowpass_enable>0
        [EMG_Blow,EMG_Alow] = ellip(7,1,60, EMG_LP_Fc/(SampFreq1/2));   % Default is OFF
        filtered_samples = filtfilt(EMG_Blow,EMG_Alow, filtered_samples);
...
elseif isequal(FileFlag, 6) % DSI EDF
    waitbar(0.1,waithandle,'Loading Movement Data ...');
    figure(waithandle),pause(0.2),
    [~, ~, edfSignal] = blockEdfLoad(filename);
    Fs = 10;
    edfStartIdx = lowertimestamp * Fs +1;
    edfStopIdx = (uppertimestamp + 1) * Fs;
    
    maxEMG = max(abs(edfSignal{1,1}));
    minEMG = -1 * maxEMG;
    set(handles.ymin1, 'String', minEMG);
    set(handles.ymax1, 'String', maxEMG);
    samples = edfSignal{1,1}(edfStartIdx:edfStopIdx);
    EMG_SAMPLES = zeros(length(samples)*100,1);
    for i = 1:length(samples)
        EMG_SAMPLES((100*(i-1)+1):100*i) = samples(i);
    end
    EMG_TIMESTAMPS = lowertimestamp:(1/1000):((uppertimestamp + 1)-1/1000);
end
...
if isequal(FileFlag, 2) % AD System
    ...
    filtered_samples=filtfilt(Blow,Alow,EEG_SAMPLES/4);
    ...
    if EEG_highpass_enable>0
        ...
        filtered_samples = filtfilt(EEG_Bhi,EEG_Ahi, filtered_samples);
    end
...
elseif isequal(FileFlag, 1) % Neuralynx System
    ...
    if EEG_highpass_enable>0  % OPTIONAL highpass filter for EEG signals
        %Design bandpass filter:
        [z, p, k] = ellip(7,1,60, [EEG_HP_Fc EEG_Fc]/(SampFreq2/2),'bandpass');   % Default is OFF
    else
        %  Low pass filter for EEG signals
        [z, p, k] = ellip(7,1,60, EEG_Fc/(SampFreq2/2));           % Default setting implements low pass filter with 30hz cutoff
    end
    [sos, g] = zp2sos(z,p,k);
    filtered_samples = filtfilt(sos,g, EEG_SAMPLES);
    %  OPTIONAL 60Hz Notch filter for EEG signals
    if EEG_Notch_enable > 0
        wo = 60/(SampFreq2/2);
        [B_EEG_Notch,A_EEG_Notch] =  iirnotch(wo, wo/35);   % Default is OFF
...
elseif isequal(FileFlag, 3) % ASCII - Polysmith
    ...
    filtered_samples=filtfilt(Blow,Alow,EEG_SAMPLES);
    %  OPTIONAL highpass filter for EEG signals
    if EEG_highpass_enable>0
        [EEG_Bhi,EEG_Ahi] = ellip(7,1,60, EEG_HP_Fc/(SampFreq1/2),'high');   % Default is OFF
        filtered_samples = filtfilt(EEG_Bhi,EEG_Ahi, filtered_samples);
...
elseif isequal(FileFlag, 6) % DSI EDF
    waitbar(0.1,waithandle,'Loading EEG Data ...');
    figure(waithandle),pause(0.2),
    Fs = 1000;
    edfStartIdx = lowertimestamp * Fs +1;
    edfStopIdx = (uppertimestamp + 1) * Fs;
    
    EEG_SAMPLES = edfSignal{1,2}(edfStartIdx:edfStopIdx);    
%     EEG_SAMPLES = EEG_SAMPLES*1000000; %Convert from volts to microvolts
    if EEG_highpass_enable>0  % OPTIONAL highpass filter for EEG signals
        %Design bandpass filter:
        [z, p, k] = ellip(7,1,60, [EEG_HP_Fc EEG_Fc]/(Fs/2),'bandpass');   % Default is OFF
    else
        %  Low pass filter for EEG signals
        [z, p, k] = ellip(7,1,60, EEG_Fc/(Fs/2));           % Default setting implements low pass filter with 30hz cutoff
    end
    [sos, g] = zp2sos(z,p,k);
    EEG_SAMPLES = filtfilt(sos,g, EEG_SAMPLES);
    %  OPTIONAL 60Hz Notch filter for EEG signals
    if EEG_Notch_enable > 0
        wo = 60/(Fs/2);
        [B_EEG_Notch,A_EEG_Notch] =  iirnotch(wo, wo/35);   % Default is OFF
        EEG_SAMPLES = filtfilt(B_EEG_Notch,A_EEG_Notch, EEG_SAMPLES);
    end
    maxEEG = max(abs(EEG_SAMPLES));
    minEEG = -1 * maxEEG;
    set(handles.ymin2, 'String', minEEG);
    set(handles.ymax2, 'String', maxEEG);
    EEG_TIMESTAMPS = EMG_TIMESTAMPS;
end
clear filtered_samples samples SF
...
if Input3_enable == 1
    ...
    if isequal(FileFlag, 2) % AD System
        ...
        if Input3_HP_enable>0
            ...
            filtered_samples = filtfilt(Input3_Bhi,Input3_Ahi, filtered_samples);
        end
        ...
        if Input3_LP_enable>0
            ...
            filtered_samples = filtfilt(Input3_Blow,Input3_Alow, filtered_samples);
        end
    ...
    elseif isequal(FileFlag, 1) % Neuralynx System
        ...
        %  OPTIONAL highpass filter for Input 3 signals
        if Input3_HP_enable>0 && Input3_LP_enable>0
            %Design bandpass filter:
            [z, p, k] = ellip(7,1,60, [Input3_HP_Fc Input3_LP_Fc]/(SampFreq3/2),'bandpass');   % Default is OFF
        elseif Input3_HP_enable>0
            %Design high pass filter:
            [z, p, k] = ellip(7,1,60, Input3_HP_Fc/(SampFreq3/2),'high');
        elseif Input3_LP_enable>0
            %Design low pass filter:
            [z, p, k] = ellip(7,1,60, Input3_LP_Fc/(SampFreq3/2),'low');
        end
        [sos, g] = zp2sos(z,p,k);
        filtered_samples = filtfilt(sos,g, INPUT3_SAMPLES);
...
%         exactHi = (exactLow + exactHi - 1);
    ...
    elseif isequal(FileFlag, 5) % Plexon
        ...
        if Input3_HP_enable>0
            ...
            INPUT3_SAMPLES = filtfilt(Input3_Bhi,Input3_Ahi, INPUT3_SAMPLES);
        end
        %  OPTIONAL lowpass filter for Input 3 signals
        if Input3_LP_enable>0
            [Input3_Blow,Input3_Alow] = ellip(7,1,60, Input3_LP_Fc/(SampFreq3/2));   % Default is OFF
            INPUT3_SAMPLES = filtfilt(Input3_Blow,Input3_Alow, INPUT3_SAMPLES);
        end
        %  OPTIONAL 60Hz Notch filter for Input 3 signals
        if Input3_Notch_enable > 0
            wo = 60/(SampFreq3/2);
            [B_Input3_Notch,A_Input3_Notch] =  iirnotch(wo, wo/35);   % Default is OFF
            INPUT3_SAMPLES = filtfilt(B_Input3_Notch,A_Input3_Notch, INPUT3_SAMPLES);
        end
        INPUT3_TIMESTAMPS = INPUT3_TIMESTAMPS(1:sampfactor:end);
        INPUT3_SAMPLES = INPUT3_SAMPLES(1:sampfactor:end);  
    
    elseif isequal(FileFlag, 6) % DSI EDF
        waitbar(0.1,waithandle,'Loading Temperature Data ...');
        figure(waithandle),pause(0.2),
        Fs = 10;
        edfStartIdx = lowertimestamp * Fs +1;
        edfStopIdx = (uppertimestamp + 1) * Fs;
        samples = edfSignal{1,3}(edfStartIdx:edfStopIdx);
        INPUT3_SAMPLES = zeros(length(samples)*100,1);
        for i = 1:length(samples)
            INPUT3_SAMPLES((100*(i-1)+1):100*i) = samples(i);
        end
        INPUT3_SAMPLES = INPUT3_SAMPLES * 200; % Not sure this is correct 
        maxTemp = max(INPUT3_SAMPLES);
        minTemp = min(INPUT3_SAMPLES);
        set(handles.ymin3, 'String', minTemp);
        set(handles.ymax3, 'String', maxTemp);
        INPUT3_TIMESTAMPS = EMG_TIMESTAMPS;
    end
clear filtered_samples samples SF
end
...
if Input4_enable>0
    ...
    if isequal(FileFlag, 2) % AD System
        ...
        if Input4_HP_enable>0
            ...
            filtered_samples = filtfilt(Input4_Bhi,Input4_Ahi, filtered_samples);
        end
        ...
        if Input4_LP_enable>0
            ...
            filtered_samples = filtfilt(Input4_Blow,Input4_Alow, filtered_samples);
        end
    ...
    elseif isequal(FileFlag, 1) % Neuralynx System
        ...
        %  OPTIONAL highpass filter for Input 4 signals
        if Input4_HP_enable>0 && Input4_LP_enable>0
            %Design bandpass filter:
            [z, p, k] = ellip(7,1,60, [Input4_HP_Fc Input4_LP_Fc]/(SampFreq4/2),'bandpass');   % Default is OFF
        elseif Input4_HP_enable>0
            %Design high pass filter:
            [z, p, k] = ellip(7,1,60, Input4_HP_Fc/(SampFreq4/2),'high');
        elseif Input4_LP_enable>0
            %Design low pass filter:
            [z, p, k] = ellip(7,1,60, Input4_LP_Fc/(SampFreq4/2),'low');
        end
        [sos, g] = zp2sos(z,p,k);
        filtered_samples = filtfilt(sos,g, INPUT4_SAMPLES);
...
%         sampfactor = 1;
    ...
    elseif isequal(FileFlag, 5) % Plexon
        ...
        if Input4_HP_enable>0
            ...
            INPUT4_SAMPLES = filtfilt(Input4_Bhi,Input4_Ahi, INPUT4_SAMPLES);
        end
        %  OPTIONAL lowpass filter for Input 4 signals
        if Input4_LP_enable>0
            [Input4_Blow,Input4_Alow] = ellip(7,1,60, Input4_LP_Fc/(SampFreq4/2));   % Default is OFF
            INPUT4_SAMPLES = filtfilt(Input4_Blow,Input4_Alow, INPUT4_SAMPLES);
        end
        %  OPTIONAL 60Hz Notch filter for Input 4 signals
        if Input4_Notch_enable > 0
            wo = 60/(SampFreq4/2);
            [B_Input4_Notch,A_Input4_Notch] =  iirnotch(wo, wo/35);   % Default is OFF
            INPUT4_SAMPLES = filtfilt(B_Input4_Notch,A_Input4_Notch, INPUT4_SAMPLES);
        end
        INPUT4_TIMESTAMPS = INPUT4_TIMESTAMPS(1:sampfactor:end);
        INPUT4_SAMPLES = INPUT4_SAMPLES(1:sampfactor:end);  
    
    elseif isequal(FileFlag, 6) % DSI EDF
        waitbar(0.1,waithandle,'Loading INPUT 4 Data ...');
        figure(waithandle),pause(0.2),
        Fs = 1000;
        edfStartIdx = lowertimestamp * Fs +1;
        edfStopIdx = (uppertimestamp + 1) * Fs;

        INPUT4_SAMPLES = edfSignal{1,2}(edfStartIdx:edfStopIdx);    
%         INPUT4_SAMPLES = INPUT4_SAMPLES*1000000; %Convert from volts to microvolts
        ...
        if Input4_HP_enable>0 && Input4_LP_enable>0
            %Design bandpass filter:
            [z, p, k] = ellip(7,1,60, [Input4_HP_Fc Input4_LP_Fc]/(Fs/2),'bandpass');   % Default is OFF
        elseif Input4_HP_enable>0
            %Design high pass filter:
            [z, p, k] = ellip(7,1,60, Input4_HP_Fc/(Fs/2),'high');
        elseif Input4_LP_enable>0
            %Design low pass filter:
            [z, p, k] = ellip(7,1,60, Input4_LP_Fc/(Fs/2),'low');
        end
        [sos, g] = zp2sos(z,p,k);
        INPUT4_SAMPLES = filtfilt(sos,g, INPUT4_SAMPLES);
        ...
        INPUT4_TIMESTAMPS = EMG_TIMESTAMPS; 
        maxEEG = max(abs(INPUT4_SAMPLES));
        minEEG = -1 * maxEEG;
        set(handles.ymin4, 'String', minEEG);
        set(handles.ymax4, 'String', maxEEG);
    end
clear filtered_samples samples SF
end
clear edfSignal % This is in case a DSI EDF data set was loaded
...
current_dir='C:\';
cd(current_dir);
[filename, pathname] = uigetfile({'*.dat;*.ascii;*.ncs;*.emg;*.stim;*.edf',...
        'All data files (*.ascii, *.dat, *.emg, *.ncs, *.stim, *.edf)'},'Pick an EMG data file');
...
current_dir='C:\';
cd(current_dir);
[filename, pathname] = uigetfile({'*.dat;*.ascii;*.ncs;*.eeg;*.edf',...
        'All data files (*.dat, *.ascii, *.ncs, *.eeg, *.edf)'}, 'Pick an EEG data file');
if isequal(filename,0) || isequal(pathname,0)
    uiwait(errordlg('You need to select a file. Please press the button again',...
        'ERROR','modal'));
    cd(working_dir);
...
cd(current_dir);
[filename, pathname] = uigetfile({'*.dat;*.ascii;*.ncs;*.eeg;*.edf',...
        'All data files (*.dat, *.ascii, *.ncs, *.eeg, *.edf)'}, 'Pick a file for Input 3');
if isequal(filename,0) || isequal(pathname,0)
    uiwait(errordlg('You need to select a file. Please press the button again',...
        'ERROR','modal'));
    cd(working_dir);
...
cd(current_dir);
[filename, pathname] = uigetfile({'*.dat;*.ascii;*.ncs;*.eeg;*.edf',...
        'All data files (*.dat, *.ascii, *.ncs, *.eeg, *.edf)'}, 'Pick a file for Input 4');
if isequal(filename,0) || isequal(pathname,0)
    uiwait(errordlg('You need to select a file. Please press the button again',...
        'ERROR','modal'));
    cd(working_dir);
